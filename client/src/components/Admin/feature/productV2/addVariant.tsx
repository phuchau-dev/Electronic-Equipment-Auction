...
const AddVariant: React.FC = () => {
  ...
  const [selectedRam, setSelectedRam] =
    useState<SingleValue<RAM>>(null);
  const [selectedScreen, setSelectedScreen] =
    useState<SingleValue<SCREEN>>(null);
  const [selectedCPU, setSelectedCPU] =
    useState<SingleValue<CPU>>(null);
  const [selectedCard, setSelectedCard] =
    useState<SingleValue<GRAPHICSCARD>>(null);
  const [selectedBattery, setSelectedBattery] =
    useState<SingleValue<BATTERY>>(null);
  const [selectedOS, setSelectedOs] =
    useState<SingleValue<OPERATINGSYSTEM>>(null);
  const [selectedStorage, setSelectedStorage] =
    useState<SingleValue<STORAGE>>(null);
  const onRamChange = (
    selectedOptions: SingleValue<RAM>,
  ) => {
    handleRamChange(
      selectedOptions,
      setSelectedRam,
  ...
  const onScreenChange = (
    selectedOptions: SingleValue<SCREEN>,
  ) => {
    handleScreenChange(
      selectedOptions,
      setSelectedScreen,
  ...
  const onCPUChange = (
    selectedOptions: SingleValue<CPU>,
  ) => {
    handleCPUChange(
      selectedOptions,
      setSelectedCPU,
  ...
  const onCardChange = (
    selectedOptions: SingleValue<GRAPHICSCARD>,
  ) => {
    handleCardChange(
      selectedOptions,
      setSelectedCard,
  ...
  const onBatteryChange = (
    selectedOptions: SingleValue<BATTERY>,
  ) => {
    handleBatteryChange(
      selectedOptions,
      setSelectedBattery,
  ...
  const onOsChange = (
    selectedOptions: SingleValue<OPERATINGSYSTEM>,
  ) => {
    handleOsChange(
      selectedOptions,
      setSelectedOs,
  ...
  const onStorageChange = (
    selectedOptions: SingleValue<STORAGE>,
  ) => {
    handleStorageChange(
      selectedOptions,
      setSelectedStorage,
  ...
  const submitFormAdd: SubmitHandler<
    ProductVariant
  > = async (data) => {
    setIsLoading(true);
    ...
    try {
      ...
      setTimeout(() => {
        navigate(
          `/admin/list-product-variant/${productId}`,
        );
      }, 2000);
    } catch (error) {
  ...
  return (
    ...
    >
      <ToastContainer />
      <ReusableBreadcrumb
        items={breadcrumbItems.addVariant}
      />
            ...
            <div className="grid grid-cols-6 gap-6">
              ...
              <FormInput
                ...
                validation={{
                  ...
                  min: {
                    value: 1000,
                    message:
                      'Giá sản phẩm không thể thấp hơn 1000',
                  },
                  max: {
                    value: 2000000000,
                    message:
                      'Giá sản phẩm không thể vượt quá 2000000000',
                  },
                  valueAsNumber: true,
                }}
              ...
              <div className="col-span-6 1sm:col-span-3">
                <FormSelect
                  ...
                  register={register}
                  validation={{
                    required: 'Giảm giá là bắt buộc',
                  }}
                  errorMessage={
