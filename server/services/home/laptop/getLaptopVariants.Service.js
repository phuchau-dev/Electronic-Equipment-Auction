...
const getLaptopVariantsService = {
  getLaptopVariants: async (page = 1, limit = 10) => {
    try {
      ...
      const category = await ProductType.findOne({ slug: 'laptop' }).lean();
      if (!category) {
        return {
          success: false,
          err: 1,
...
module.exports = getLaptopVariantsService;
